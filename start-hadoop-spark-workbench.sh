...
if [ $FORCE_RECREATE -eq 0 ]; then
    DOCKER_CMD="docker-compose -f ${DOCKER_FILE} up -d --user=$UID --remove-orphans --no-recreate"
else
    DOCKER_CMD="docker-compose -f ${DOCKER_FILE} up -d --user=$UID --remove-orphans --force-recreate"
fi
#if [ $BUILD -eq 0 ]; then
